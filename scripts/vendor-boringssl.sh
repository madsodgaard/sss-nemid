#!/bin/bash
# This was substantially adapted from SwiftCrypto's/JWTKit vendor-boringssl.sh script.
...
#      a local copy of the BoringSSL sources in Sources/CNemIDBoringSSL.
#      Any prior contents of Sources/CNemIDBoringSSL will be deleted.
#
set -eou pipefail

HERE=$(pwd)
DSTROOT=Sources/CNemIDBoringSSL
...
function mangle_symbols {
    ...
    (
        ...
        swift build --product CNemIDBoringSSL --enable-test-discovery
        go run "${SRCROOT}/util/read_symbols.go" -out "${TMPDIR}/symbols-macOS.txt" "${HERE}/.build/debug/libCNemIDBoringSSL.a"
        ...
        # xcodebuild -sdk iphoneos -scheme CNemIDBoringSSL -derivedDataPath "${TMPDIR}/iphoneos-deriveddata"
        # ar -r "${TMPDIR}/libCNemIDBoringSSL-ios.a" "${TMPDIR}/iphoneos-deriveddata/Build/Products/Debug-iphoneos/CNemIDBoringSSL.o"
        # go run "${SRCROOT}/util/read_symbols.go" -out "${TMPDIR}/symbols-iOS.txt" "${TMPDIR}/libCNemIDBoringSSL-ios.a"
        ...
        for cc_target in "${CROSS_COMPILE_TARGET_LOCATION}"/*"${CROSS_COMPILE_VERSION}"*.json; do
            ...
            swift build --product CNemIDBoringSSL --destination "${cc_target}" --enable-test-discovery
        done;

        # Now we need to generate symbol mangles for Linux. We can do this in
        # one go for all of them.
        go run "${SRCROOT}/util/read_symbols.go" -obj-file-format elf -out "${TMPDIR}/symbols-linux-all.txt" "${HERE}"/.build/*-unknown-linux/debug/libCNemIDBoringSSL.a
    ...
    perl -pi -e '$_ .= qq(\n#define BORINGSSL_PREFIX CNemIDBoringSSL\n) if /#define OPENSSL_HEADER_BASE_H/' "$DSTROOT/include/openssl/base.h"

    for assembly_file in $(find "$DSTROOT" -name "*.S")
    do
        $sed -i '1 i #define BORINGSSL_PREFIX CNemIDBoringSSL' "$assembly_file"
    done
...
(
    ...
    # - Their names will be prefixed with 'CNemIDBoringSSL_'
    # - The headers prefixed with 'boringssl_prefix_symbols' will also be prefixed with 'CNemIDBoringSSL'
    ...
    # Now change the imports from "<openssl/X> to "<CNemIDBoringSSL_X>", apply the same prefix to the 'boringssl_prefix_symbols' headers.
    find . -name "*.[ch]" -or -name "*.cc" -or -name "*.S" | xargs $sed -i -e 's+include <openssl/+include <CNemIDBoringSSL_+' -e 's+include <boringssl_prefix_symbols+include <CNemIDBoringSSL_boringssl_prefix_symbols+'

    # Okay now we need to rename the headers adding the prefix "CNemIDBoringSSL_".
    pushd include
    find . -name "*.h" | $sed -e "s_./__" | xargs -I {} mv {} CNemIDBoringSSL_{}
    ...
    find . -name "*.h" | xargs $sed -i -e 's/include "/include "CNemIDBoringSSL_/' -e 's/include <CNemIDBoringSSL_\(.*\)>/include "CNemIDBoringSSL_\1"/'
    popd
)
...
cat << EOF > "$DSTROOT/include/CNemIDBoringSSL.h"
